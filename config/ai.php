...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | The providers an agent should fall back to, in order, when the default
    | provider is rate limited, overloaded or out of credit. Each provider
    | listed here needs its own key configured below to be usable.
    |
    */

    'failover' => array_values(array_filter(
        array_map('trim', explode(',', (string) env('AI_FAILOVER_PROVIDERS'))),
    )),
